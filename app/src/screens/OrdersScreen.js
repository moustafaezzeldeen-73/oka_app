...
export default function OrdersScreen() {
   ...
   * Kept in step with `isShipped` in server/integrations/shopify.js, which enforces it.
   */
  ...
  const loadStatus = useCallback(async () => {
    ...
    setLive(null);
    // ordersVersion isn't read inside: bumping it after an edit or cancel is
    // what makes this refetch.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state.session?.token, d.lang, state.selectedOrderName, state.ordersVersion, actions]);
